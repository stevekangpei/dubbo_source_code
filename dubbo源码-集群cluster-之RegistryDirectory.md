# dubbo源码-集群cluster-之RegistryDirectory


```
（1）RegistryDirectory, 在dubbo-registry模块中， Dubbo 通过注册中心模块集成 Directory 的实现类。
（2）RegistryDirectory 实现了NotifyListener ，订阅注册中心( Registry ) 的数据，实现对变更的监听。
```

#### 成员变量和构造方法
```java
    // Dubbo SPI Adaptive对象
...
#### 说明：
> 总共有5类变量，
* （1） 主要是Dubbo SPI相关的参数，如Cluster，Router，Configurator，负责集群容错，路由，配置
* （2） 服务消费者相关的参数
* （3） 服务注册中心相关的参数
* （4） 配置规则相关
* （5） 服务provider相关的参数
* （6） RegistryDirectory 实现了NotifyListener接口，意味着他也是一个监听器，当变更发生的时候，注册中心会通知RegistryDirectory 做相应的变更。
 ...
 * （2）将 `urls` 按照 `url.parameter.category` 分类，添加到集合result 中
 * （3）针对result map，分类调用listener 进行notify处理。
 * （4）最后一行代码 listener.notify(categoryList); 调用子类，这个地方会调用到
      RegistryDirectory的notify方法进行处理。


#### com.alibaba.dubbo.registry.integration.RegistryDirectory#notify方法：

```java
    /**
     *  (1) 将传来的协议分为三个组合，分别是 router，configurator，invoker。
     * （2） 如果configurators非空，那么更新configurators (参考toConfigurators)
     *  (3) 如果routersUrl列表非空，则更新 routers （参考toRouters）
     * （4） 合并配置规则，到 `directoryUrl` 中，形成 `overrideDirectoryUrl` 变量
     * （5） 由于有URL变更，所以需要刷新Invoker列表。
     * 
...
```java
     ...
     *  
     *  （1）如果是 EMPTY_PROTOCOL，则直接清空配置项
     *  （2）对应第 4 条契约，不带参数的 override://0.0.0.0/ 表示清除 override
     *  （3）通过RegistryDirectory里面的 configuratorFactory 通过URL 获取最新的configurator。
     *  （4）更新configurator列表
    /**
...
```java
    /**
     * （1）如果是以EMPTY_PROTOCOL 开头的话，表示忽略
     * （2）更新router的协议类型
     * （3）通过routerFactory 工厂获取router
     * （4）更新router
     */
...
```java
     ...
     *
     * （1）如果是EMPTY_PROTOCOL 开头的话，说明要禁止访问invoker，需要设置禁止访问，同时情况invoker集合
     * （2）允许访问，先找到合适的invokerUrl集合。
     *      （2.1） 如果invokerUrls 是空的，说明是路由规则或配置规则发生改变，直接使用 cachedInvokerUrls 。同时直接返回，不需要更新invoker。
     *      （2.2） 如果非空，则需要更新cachedInvokerUrls 
     *      （2.3） 将传入的invokerUrl 列表转换为 invoker集合  #toInvokers(invokerUrls);
     *      （2.4） 转换为新的 newMethodInvokerMap #toMethodInvokers(newUrlInvokerMap)
     *      （2.5） 更新成员变量里面的 methodInvokerMap 和  urlInvokerMap
     *      （2.6） 销毁不在使用的 Invoker 集合 #destroyUnusedInvokers(oldUrlInvokerMap, newUrlInvokerMap);
...
#### toInvokers方法：
  ...
  ```java
     ...
     *  
     *  整体逻辑是根据当前提供的URL 找到满足匹配的协议的URL。然后合并URL参数，从 urlInvokerMap 中查询，如果查到了，就不用管，否则重新
     *  refer一个新的invoker，更新URLMap
     * 
     * （1）从queryMap中找到协议对应的值，
     * （2）判断当前提供的URL中 有么有和协议对应的URL，没有直接跳过
     * （3）如果协议 以EMPTY_PROTOCOL 为开头，直接跳过
     * （4）若应用程序不支持该协议，则直接跳过， （ExtensionLoader.getExtensionLoader(Protocol.class).hasExtension）
     * （5）合并URL参数，并转为key
     * （6）如果 urlInvokerMap 缓存中包含这个key的话，则直接更新  newUrlInvokerMap 即可
     * （7）如果不包含，说明URL参数已经改变了，如果如果协议是enable = true的话，则重新refer一个新的invoker 更新newUrlInvokerMap
     * （8）返回URLMap
     * 
     *
     * @param urls URL 集合
...
#### mergeUrl方法：
  ```java
      /**
     * (1) 首先 合并消费端参数  ClusterUtils.mergeUrl 比较简单不做解析
     * （2）通过 configurators 更新providerUrl
     * （3）最后增加 CHECK_KEY 
     * （4）返回 providerUrl
     */
...
```java
     ...
     *
     * （1）通过 getUrl().getParameter  获取方法值
     * （2）将invokersMap 转换为 方法对应的 Map<String, List<Invoker<T>>> newMethodInvokerMap 其中key 为method名称
     * （3）路由全 `invokersList` ，匹配合适的 Invoker 集合 注意这个时候 method = null，表示是服务提供者的全量 Invoker 集合，这个时候 key 为 ANY_VALUE
     * （4）对每个method 对应的invokerList， 调用路由方法进行路由，找到方法对应的子项invokerList。
     * （5）循环排序每个方法的 Invoker 集合，并设置为不可变
     * 
     * 
...
```java
     ...
     *
     * （1）对比新老集合，如果 newInvokers 不包含oldInvoker 则添加到 deleted 列表
     * （2）一个个 invoker.destroy();。
     * 
...
```
上面全都是基本的辅助性方法，比如说什么时候notify更新，
如何更新invoker，如何close等等。

Directory的核心方法还是获取InvokerList
```

```java
    /**
     * （1） 如果 forbidden 则直接返回
     * （2） 根据方法名 + 第一个参数枚举路由 找到invokerList
     * （3） 如果为null的话，直接根据方法名 获取
     * （4） 如果为null的话，通过 ANY_VALUE获取  比如说回声方法  `#$echo(name)` ，回声方法
     * （5） 如果 还为null的话，去第一个invoker集合。
     * 
